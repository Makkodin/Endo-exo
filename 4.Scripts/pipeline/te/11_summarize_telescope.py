...
def build_overview(sample: str, out: pd.DataFrame, class_summary: pd.DataFrame) -> dict:
    top = out.sort_values("telescope_count", ascending=False).iloc[0].to_dict() if not out.empty else {}
    row = {
        "sample_id": sample,
        "telescope_status": "completed",
        "telescope_total_assigned_count": float(out["telescope_count"].sum()) if not out.empty else 0.0,
        "telescope_top_locus": top.get("locus_id", ""),
        "telescope_top_count": top.get("telescope_count", 0),
        "telescope_top_class": top.get("te_class_group", ""),
        "telescope_top_family": top.get("repeat_family", ""),
        "telescope_top_repeat_name": top.get("repeat_name", ""),
    }
    for cls in CLASS_ORDER:
        key = cls.lower().replace("simple_repeat", "simple_repeat").replace("low_complexity", "low_complexity")
        if cls == "LTR":
            prefix = "telescope_herv_erv_ltr"
        else:
            prefix = f"telescope_{key}"
        part = class_summary[class_summary["te_class_group"] == cls]
        item = part.iloc[0].to_dict() if not part.empty else {}
        row[f"{prefix}_assigned_count"] = item.get("total_telescope_count", 0)
        row[f"{prefix}_rpm"] = item.get("telescope_rpm", 0)
        row[f"{prefix}_top_family"] = item.get("top_family", "")
        row[f"{prefix}_top_repeat_name"] = item.get("top_repeat_name", "")
        row[f"{prefix}_top_locus"] = item.get("top_locus", "")
        row[f"{prefix}_top_count"] = item.get("top_locus_count", 0)
    return row
...
def main() -> None:
    ...
    df = read_tsv_or_empty(args.report)
    ...
    out.to_csv(out_dir / f"{args.sample}.telescope_counts.normalized.tsv", sep="\t", index=False)
    ...
    pd.DataFrame([build_overview(args.sample, out, class_summary)]).to_csv(out_dir / f"{args.sample}.telescope_overview.tsv", sep="\t", index=False)
